...
TEST_CASE("Negative Number Works", "[negative]") {
    REQUIRE(add(-2, -3) == -5);
    REQUIRE(subtract(-5, -3) == -2);
    REQUIRE(multiply(-2, -4) == 8);
    REQUIRE(divide(-10, 2) == Catch::Approx(-5.0));
...
TEST_CASE("Exponentiation works", "[exponentiate]") {
    REQUIRE(exponentiate(5.0, 0) == Catch::Approx(1.0));
    REQUIRE(exponentiate(2.0, 4) == Catch::Approx(16.0));
}